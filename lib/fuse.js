...
var submitToFuse = function(submission, cb){
  ...
  var url = process.env.FUSE_URL || 'http://85.190.180.161:8182/cxf/redhat/forum/register';
  var options = { "url" : url, "body": data, "json": true};
  request.post(options, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      console.log("Well done!") // Show the HTML
    }
...
var transform = function(submission) {
  var map = { "register" : { register : {} } };
  _.each(submission.submission.formFields, function(item){
    //console.log(JSON.stringify(item));
    map.register[mapping[item.fieldId.fieldCode]] = item.fieldValues[0];
  });
